    ...
    {\n\t\t" . '$this' . "->app->singleton('$name', function () {\n\t\t\treturn new $name(new $model);\n\t\t});\n"],
            ...
            if (!File::exists(app_path('/Providers/EloquentServiceProvider.php'))) {
                if ($this->confirm("The service provider for eloquent repositories does not exist! Do you want to create it?")) {
                    $defaultProviderContent = File::get(app_path('/Console/Commands/Stubs/EloquentServiceProvider.stub'));
                    $runtimeProviderContent = str_replace('Dummy', $model, $defaultProviderContent);
                    File::put(app_path('/Console/Commands/Stubs/EloquentServiceProvider.stub'), $runtimeProviderContent);
                    File::copy(app_path('/Console/Commands/Stubs/EloquentServiceProvider.stub'), app_path('/Providers/EloquentServiceProvider.php'));
                    File::put(app_path('/Console/Commands/Stubs/EloquentServiceProvider.stub'), $defaultProviderContent);
        ...
        $defaultFacadeContent = File::get(app_path('/Console/Commands/Stubs/DummyFacade.stub'));
        $runtimeFacadeContent = str_replace('DummyFacade', $name, $defaultFacadeContent);
        File::put(app_path('/Console/Commands/Stubs/DummyFacade.stub'), $runtimeFacadeContent);
        File::copy(app_path('/Console/Commands/Stubs/DummyFacade.stub'), app_path('/Facades/' . $name . '.php'));
        File::put(app_path('/Console/Commands/Stubs/DummyFacade.stub'), $defaultFacadeContent);
